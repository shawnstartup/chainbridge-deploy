...
const deployCmd = new Command("deploy")
    ...
    .action(async (args) => {
        ...
        if (args.all) {
            await deployBridgeContractEstimate(args);

            const { doDeploy } = await Inquirer.default.prompt([
                { type: 'text', message: 'continue deploying(y/n):', name: 'doDeploy' },
            ]);

            if (doDeploy !== 'y') {
                console.log("Deploying is stopped");
                return;
            }
            console.log("Deploying continue ...");
...
async function deployBridgeContractEstimate(args) {
    ...
    const deployment = factory.getDeployTransaction(
        ...
        args.expiry,);
    const gasEstimate = await args.provider.estimateGas(deployment);
    console.log('Estimated gas:', gasEstimate.toString());
    const gasPrice = await args.provider.getGasPrice();
    console.log('Gas Price:', gasPrice.toString());

    const actualCost = gasEstimate.mul(gasPrice);
    console.log('Actual Cost (in wei):', actualCost.toString());
    const actualCostInEth = ethers.utils.formatEther(actualCost);
    console.log('Actual Cost (in ETH):', actualCostInEth);
}
